var radioAlert = document.getElementById("NavBar");
radioAlert.innerHTML = '<header class="clear" id="header">'+
        '<div class="fl_left" id="logo"><h1><a href="/"> <img src="/images/TEW_ICON.PNG" alt="Toshi Engineering Works"></a></h1></div>'+
        '<nav class="fl_right" id="mainav"><ul class="clear">'+
            '<li><a href="/">Home</a></li><li><a href="/about">About Us</a></li>'+
            '<li value="Products"><a class="drop" href="#">Products</a>'+
              '<ul style="width:250px">'+
                '<li><a class="drop" href="#">Tube Mills</a>'+
                  '<ul style="width:250px;left:240px">'+
                    '<li><a href="/product/Carbon Steel Tube Mill">Carbon Steel Tube Mill</a></li>'+
                    '<li><a href="/product/Stainless Steel Tube Mill">Stainless Steel Tube Mill</a></li>'+
                    '<li><a href="/product/ERW High Speed Tube Mill">ERW High Speed Tube Mill</a></li>'+
                    '<li><a href="/product/API Tube Mill">API Tube Mill</a></li>'+
                  '</ul>'+
                '</li>'+
                '<li><a href="/product/Rolls">Rolls</a></li>'+
                '<li><a href="/product/Horizontal Strip Accumulator">Horizontal Strip Accumulator</a></li>'+
                '<li><a href="/product/Cut To Length">Cut To Length</a></li>'+
                '<li><a href="/product/High Frequency Welder">High Frequency Welder</a>'+
                '</li>'+
                '<li><a class="drop" href="#">Cold Saw </a>'+
                  '<ul style="width:250px;left:240px">'+
                    '<li><a href="/product/NC Flying Cold Saw Cut-Off">NC Flying Cold Saw Cut-Off</a></li>'+
                    '<li><a href="/product/Burrfree Cold Saw">Burrfree Cold Saw </a></li>'+
                  '</ul>'+
                '</li>'+
                '<li><a href="/product/Slitting Machine">Slitting Machine HR/CR/SS</a></li>'+
                '<li><a href="/product/Tube Hydro Tester">Tube Hydro Tester</a></li>'+
                '<li><a href="/product/Automatic End Facing Machine">Automatic End Facing Machine</a></li>'+
                '<li><a href="/product/Tube Straightening Machine">Tube Straightening Machine</a></li>'+
                '<li><a href="/product/Threading Machine">Threading Machine </a></li>'+
                '<li><a class="drop" href="#">Galvanizing Plant</a>'+
                  '<ul style="width:250px;left:240px">'+
                    '<li><a href="/product/Galvanizing Structure Plant">Galvanizing Structure Plant</a></li>'+
                    '<li><a href="/product/Automatic Hot Dip Galvanizing Plant">Automatic Hot Dip Galvanizing Plant</a></li>'+
                  '</ul>'+
                '</li>'+
                '<li><a href="/product/Pollution Plant">Pollution Plant</a></li>'+
              '</ul>'+
            '</li>'+
            '<li><a class="drop" href="#">Gallery</a>'+
              '<ul>'+
                '<li><a href="/gallery/photo/0">Photo</a></li>'+
                '<li><a href="/gallery/video">Video</a></li>'+
              '</ul>'+
            '</li>'+
            '<li><a href="/career">Careers</a></li>'+
            '<li><a href="/businessEnquiry">Business Enquiry</a></li>'+
            '<li><a href="/contact">Contact Us</a></li>'+
          '</ul>'+
        '</nav>'+
      '</header>';